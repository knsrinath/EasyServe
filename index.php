<?php
	...
	$theme = getenv('THEME'); // Theme names
	...
	if ($browseDirectories) {
		...
		if (!empty($_GET['b']) && $_GET['b'] != '..' && is_dir('files/' . $_GET['b'])) {
			$browseIgnored = false;
	...
	if (!empty($_browse) && $openIndex) {
		$_index = null;
		if (file_exists('files/' . $_browse . "/index.htm")) $_index = "/index.htm";
		if (file_exists('files/' . $_browse . "/index.html")) $_index = "/index.html";
		if (file_exists('files/' . $_browse . "/index.php")) $_index = "/index.php";
		if (!empty($_index)) {
	...
	$items = ls('./files' . (empty($_browse) ? '' : '/' . $_browse), $showDirectories, $showHiddenFiles);
				...
				<li class="item">
					...
					<?php
						if ($item['isdir'] && $browseDirectories && !@$item['isparent']) {
							...
							$itemURL = 'files/' . (empty($_browse) ? '' : str_replace(['%2F', '%2f'], '/', rawurlencode((string)$_browse)) . '/') . rawurlencode($item['name']);
						}
					?>
